...
**Latest Update**: System Optimizer & Performance Suite (RAM/CPU/Debloat)
...
### 2. System Optimizer (NEW in v1.19.0)
A high-performance module designed to maximize hardware efficiency:
- **RAM & CPU Smart Boost**:
    - **Live Test**: Real-time console agent that trims process working sets and lowers background CPU priorities.
    - **Permanent Service**: Installs a hidden background service using Windows Scheduled Tasks (`TGS_Booster_Service`).
- **One-Time Optimizations**:
    - **Stop Caching**: Disables `SysMain` (Superfetch) to free up physical memory.
    - **Ultimate CPU**: Forces the "Ultimate Performance" power plan and unparks all CPU cores.
    - **Visuals & Anims**: Toggles system transparency and window animations for an "instant feel" UI.

### 3. System Setup & Identity
...
| :--- | :--- |
| **v1.19.0** | System Optimizer tab, RAM/CPU Boosters, Windows Debloat tools. |
